...
const EVENT_OWNER_WRITABLE: ColumnMap = {
  title: 'title',
  description: 'description',
  date: 'event_date',
  time: 'event_time',
...
export async function fetchBusinessHome(userId: string): Promise<BusinessHome> {
  return withUserRead(userId, async (db) => {
    const rows = await db.run<{ payload: BusinessHome }>(
      `
      with mine as (
        select * from public.businesses where created_by = $1
    ...
    const payload = rows[0]?.payload ?? { business: null, offers: [], events: [] };
...
export async function createBusinessEvent(
  ...
  if (!String(data.location ?? '').trim()) throw new Error('Location is required');
  await withUser(userId, async (db) => {
    await insertRow(
      db, 'public.events',
