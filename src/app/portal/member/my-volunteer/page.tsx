  ...
  if (!myApp) {
    return (
      <div className="animate-fade-in" style={{ textAlign: 'center', padding: '80px 20px' }}>
        <HandHeart size={48} style={{ color: 'var(--gray-300)', marginBottom: 16 }} />
    ...
    <div className="animate-fade-in">
      ...
      <div className="card" style={{ marginBottom: 24, borderLeft: `4px solid ${sc.color}` }}>
        ...
        {myApp.adminNotes && myApp.status === 'approved' && (
          <div style={{ marginTop: 16, padding: 12, borderRadius: 8, background: 'var(--success-50)', border: '1px solid var(--success-50)', fontSize: '0.82rem' }}>
        ...
        <>
          <div className="mobile-stack" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
            ...
            <div className="card-stat">
              ...
              <div style={{ fontSize: '1.8rem', fontWeight: 800, color: 'var(--success-600)' }}>{myAssignments.filter(a => a.status === 'completed').length}</div>
            </div>
          </div>
              ...
              {myAssignments.map(asg => (
                <div key={asg.id} className="card" style={{ padding: '20px 24px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                    <div>
                      <span style={{ fontSize: '0.68rem', fontFamily: 'monospace', color: 'var(--text-muted)', background: 'var(--bg-secondary)', padding: '2px 6px', borderRadius: 4 }}>{asg.id}</span>
                  ...
                  <div style={{ padding: 14, borderRadius: 8, background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', marginBottom: 12 }}>
                    <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>Admin Instructions</div>
                  ...
                  {/* Privacy Notice */}
                  <div style={{ marginTop: 12, padding: '8px 12px', borderRadius: 6, background: 'rgba(232, 93, 4, 0.04)', border: '1px solid rgba(232, 93, 4, 0.1)', fontSize: '0.72rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 6 }}>
