    ...
    <div className="animate-fade-in">
      ...
      <div className="mobile-stack" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
        ...
        <div className="card-stat" style={{ borderLeft: '3px solid var(--success-600)' }}>
          ...
          <div style={{ fontSize: '1.8rem', fontWeight: 800, color: 'var(--success-600)' }}>{completed.length}</div>
        </div>
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {pendingAssignment.map(req => (
              <Link key={req.id} href={`/portal/admin/requests/${req.id}`} style={{ textDecoration: 'none' }}>
                <div className="card" style={{ padding: '16px 20px', cursor: 'pointer', borderLeft: '3px solid var(--warning-500)' }}>
      ...
      {assignments.length === 0 ? (
        <div className="card" style={{ textAlign: 'center', padding: 60, borderStyle: 'dashed' }}>
          <FolderKanban size={40} style={{ color: 'var(--gray-300)', marginBottom: 12 }} />
            ...
            <div key={asg.id} className="card" style={{ padding: '20px 24px' }}>
              ...
              <div className="mobile-stack" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                <div style={{ padding: 10, borderRadius: 8, background: 'var(--bg-secondary)', border: '1px solid var(--border-color)' }}>
                  <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Volunteer</div>
                  <div style={{ fontSize: '0.85rem', fontWeight: 600, marginTop: 2 }}>{asg.volunteerName}</div>
                </div>
                <div style={{ padding: 10, borderRadius: 8, background: 'var(--bg-secondary)', border: '1px solid var(--border-color)' }}>
                  <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Due Date</div>
                  <div style={{ fontSize: '0.85rem', fontWeight: 600, marginTop: 2 }}>{new Date(asg.dueDate).toLocaleDateString()}</div>
                </div>
                <div style={{ padding: 10, borderRadius: 8, background: 'var(--bg-secondary)', border: '1px solid var(--border-color)' }}>
                  <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Last Updated</div>
                  <div style={{ fontSize: '0.85rem', fontWeight: 600, marginTop: 2 }}>{new Date(asg.updatedAt).toLocaleDateString()}</div>
                </div>
              </div>
              {asg.volunteerResponse && (
                <div style={{ marginTop: 12, padding: 12, borderRadius: 8, background: 'var(--success-50)', border: '1px solid var(--success-50)', fontSize: '0.85rem' }}>
