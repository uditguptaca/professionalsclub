    ...
    <div className="animate-fade-in">
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={8} style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)' }}>No requests match your filters.</td></tr>
            ) : (
              filtered.map(req => (
                <tr key={req.id} style={{ borderBottom: '1px solid var(--bg-secondary)' }}>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    <span style={{
                      ...
                      background: req.urgency === 'critical' ? 'var(--error-50)' : req.urgency === 'high' ? 'var(--bg-secondary)' : req.urgency === 'medium' ? 'var(--warning-50)' : 'var(--success-50)',
                      color: req.urgency === 'critical' ? 'var(--error-600)' : req.urgency === 'high' ? '#9a3412' : req.urgency === 'medium' ? 'var(--primary-800)' : '#166534',
                    }}>{req.urgency}</span>
                  </td>
                  <td style={{ padding: '12px 16px' }}>
